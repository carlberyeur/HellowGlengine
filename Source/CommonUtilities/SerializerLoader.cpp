...
namespace CU
{
	...
	{
		memcpy(&aChar, myBuffer.AsPointer(myPointer), sizeof(unsigned char));
		myPointer += sizeof(unsigned char);
	}

	void CSerializerLoader::Cerealize(unsigned short& aShort)
	{
		memcpy(&aShort, myBuffer.AsPointer(myPointer), sizeof(unsigned short));
		myPointer += sizeof(unsigned short);
	}
	...
	void CSerializerLoader::Assign(const char aBuffer[], const unsigned int aSize)
	{
		myBuffer.Resize(aSize);
		myBuffer.AddChunk(aBuffer, aSize);
	}
}
